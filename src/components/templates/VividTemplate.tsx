...
export function VividTemplate({
  ...
  const mesh = `radial-gradient(120% 90% at 10% 0%, ${rgba(accent, 0.95)} 0%, transparent 50%),
    radial-gradient(100% 80% at 90% 10%, #EC4899 0%, transparent 45%),
    radial-gradient(80% 60% at 50% 100%, #7C3AED 0%, #1a0533 70%)`;

  return (
    <TemplateRoot fluid={fluid} className="relative min-h-screen text-white">
      ...
      <div className="relative mx-auto flex max-w-md flex-col items-center px-5 pb-14 pt-14 text-center">
        ...
        <h1 className="font-display mt-6 text-3xl font-bold tracking-tight drop-shadow-sm">{profile.fullName}</h1>
        {profile.jobTitle ? <p className="mt-1 text-sm font-medium text-white/85">{profile.jobTitle}</p> : null}
        {profile.companyName ? (
          <p className="mt-1 text-xs uppercase tracking-[0.2em] text-white/60">{profile.companyName}</p>
        ) : null}

        {profile.description ? (
          <p className="mt-4 max-w-xs text-[13px] leading-relaxed text-white/75">{profile.description}</p>
        ) : null}
        ...
        <div className="mt-7 w-full">
          <SaveContactButton
            ...
            accent="#ffffff"
            dark
            className="!text-neutral-900 !shadow-lg"
          />
        </div>

        <div className="mt-4">
          <ContactAndSocialPills profile={profile} accent="#ffffff" dark />
        </div>

        <div className="mt-6 w-full">
          <LinkList links={links} accent={accent} variant="vivid" dark />
        </div>
        ...
        <footer className="mt-10 text-[11px] text-white/45">Powered by Volt Cards</footer>
      </div>
    </TemplateRoot>
  );
}
