...
export async function createLink(input: z.infer<typeof LinkSchema>): Promise<ActionResult> {
...
export async function updateLink(id: string, input: z.infer<typeof LinkSchema>): Promise<ActionResult> {
  ...
  if (!link || link.profileId !== profile.id) return { ok: false, error: "Not found" };
  await prisma.link.update({ where: { id }, data: { label: parsed.data.label, url: parsed.data.url } });
  revalidatePath(`/${profile.slug}`);
  revalidatePath("/dashboard/links");
  return { ok: true };
}
